...
我先把图片里这 5 个问题用大白话给你解释一遍，然后再给你一套“上课直接用”的英语口语回答模板（简单、好记、地道）。

---

### 大白话理解（帮你理清逻辑，不用背）

**Q 1: "unforgotten" 有几个语素（morphemes）？**
*   **大白话**：语素就是“最小的、有意义的部分”。把词拆开。
*   **拆解**：un（表示“不”的前缀）+ forget（核心动词）+ en（表示被动/完成的词缀）。所以是 3 个。

**Q 2: 同化（assimilation）和异化（dissimilation）的区别？**
*   **大白话**：
    *   **同化（Assimilation）**：为了读得顺口，两个音变得像彼此。比如 `cats` 里的 `s` 本来发 /z/，但因为前面 `t` 是清辅音，为了省力，`s` 被同化成了清辅音 /s/。
...
老师提问时，你可以用 `"Basically..."` (基本上...), `"Let's take... for example"` (我们拿...举个例子) 来拖延时间组织语言。
...
2. **举例子**：语言学最怕干巴巴的定义。像 A 2 那样举出 `cats` 和 拉丁语的例子，老师一听就知道你懂了。
